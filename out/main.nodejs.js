...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   TempestWeatherStation: () => (/* binding */ TempestWeatherStation),
...
class TempestWeatherStation extends _scrypted_sdk__WEBPACK_IMPORTED_MODULE_0__.ScryptedDeviceBase {
    constructor(nativeId) {
        super(nativeId);
        this.temperature = 0;
        this.humidity = 0;
        ...
        this.temperatureUnit = _scrypted_sdk__WEBPACK_IMPORTED_MODULE_0__.TemperatureUnit.F;
        this.type = _scrypted_sdk__WEBPACK_IMPORTED_MODULE_0__.ScryptedDeviceType.Sensor;
        this.updateStatus();
        this.startPeriodicUpdates();
    }
    async getSettings() {
        return [
            { key: 'accessToken', title: 'Personal Access Token', value: this.storage.getItem('accessToken'), type: 'password' },
            { key: 'stationId', title: 'Station ID', value: this.storage.getItem('stationId'), type: 'string' },
            { key: 'updateInterval', title: 'Update Interval (minutes)', value: this.storage.getItem('updateInterval') || '5', type: 'number' }
        ];
    }
    async putSetting(key, value) {
        this.storage.setItem(key, value.toString());
        if (key === 'updateInterval') {
            this.startPeriodicUpdates(parseInt(value));
        }
    ...
    async updateStatus() {
        ...
        if (!accessToken || !stationId) {
            this.console.error('Access token or station ID not configured');
            return;
        }
        try {
            const response = await axios__WEBPACK_IMPORTED_MODULE_1__["default"]["default"].get(`https://swd.weatherflow.com/swd/rest/observations/station/${stationId}?token=${accessToken}`);
            ...
            this.binaryState = this.windGust >= 6; // Example threshold for binary state
            // Update Scrypted interfaces
            this.onDeviceEvent(_scrypted_sdk__WEBPACK_IMPORTED_MODULE_0__.ScryptedInterface.Thermometer, this.temperature);
            this.onDeviceEvent(_scrypted_sdk__WEBPACK_IMPORTED_MODULE_0__.ScryptedInterface.HumiditySensor, this.humidity);
            this.onDeviceEvent(_scrypted_sdk__WEBPACK_IMPORTED_MODULE_0__.ScryptedInterface.Battery, this.batteryLevel);
            this.onDeviceEvent(_scrypted_sdk__WEBPACK_IMPORTED_MODULE_0__.ScryptedInterface.BinarySensor, this.binaryState);
            // Fan interface removed. If needed, handle fan-related events differently.
            this.console.info('Weather data updated successfully');
        }
        catch (error) {
    ...
    // Implement required properties for interfaces
    async getTemperatureUnit() {
        return this.temperatureUnit;
    }
    async setTemperatureUnit(unit) {
        this.temperatureUnit = unit;
        this.storage.setItem('temperatureUnit', unit.toString());
    }
    // The Fan interface and setFan method have been removed as they are not needed for a weather station.
...
/* harmony default export */ function __WEBPACK_DEFAULT_EXPORT__(nativeId) {
